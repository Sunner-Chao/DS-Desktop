...
function createTauriBridge(): Window["deepseekDesktop"] {
  ...
  return {
    ...
    startTerminal: async (options) => {
      const result = await invoke<{ ok: boolean; error?: string; pid?: number; sessionId?: string; finalOutput?: string; exitCode?: number }>("terminal_start", {
        options: {
    ...
    getTerminalResult: (sessionId) => invoke<{ sessionId: string; exitCode: number; finalOutput: string } | null>("terminal_result", { sessionId }),
    stopTerminal: async () => {
      await invoke<void>("terminal_stop", { sessionId: activeTerminalSessionId });
      activeTerminalSessionId = "";
      return { ok: true };
    },
    sendTerminalInput: (data) => activeTerminalSessionId
      ? invoke<void>("terminal_input", { sessionId: activeTerminalSessionId, data })
      : undefined,
    resizeTerminal: (size) => activeTerminalSessionId
      ? invoke<void>("terminal_resize", { sessionId: activeTerminalSessionId, cols: size.cols, rows: size.rows })
      : undefined,
    ...
    onTerminalData: (callback) => {
      listen<string | { data?: string; sessionId?: string }>("terminal:data", (event) => {
        callback(typeof event.payload === "string" ? event.payload : event.payload.data || "");
      }).then((unlisten) => unlisten);
      return () => {};
    },
    onTerminalExit: (callback) => {
      listen<{ exitCode?: number; signal?: number; sessionId?: string; finalOutput?: string }>("terminal:exit", (event) => {
        if (event.payload.sessionId && event.payload.sessionId === activeTerminalSessionId) {
          activeTerminalSessionId = "";
        }
        callback({
