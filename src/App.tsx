...
function buildConversationPrompt(history: ChatMessage[], prompt: string, language: AppLanguage, settings?: Settings) {
  ...
  const usefulHistory = history
    ...
    .map((message) => ({
      role: message.role,
      content: stripAnsi(message.content || "").trim()
    }))
    .filter((message) => message.content && !progressPattern.test(message.content));
  ...
  if (!usefulHistory.length) return `${hintBlock}${prompt}`;

  const roleLabel = (role: ChatMessage["role"]) => {
    if (language === "zh") return role === "user" ? "用户" : "助手";
    return role === "user" ? "User" : "Assistant";
  };
  const contextHeader = language === "zh"
    ? "以下是同一聊天窗口中的最近上下文，请在回答最后一个用户问题时参考："
    : "Recent context from this same chat session. Use it when answering the final user request:";
  const finalHeader = language === "zh" ? "当前用户问题：" : "Current user request:";
  const context = usefulHistory
    .map((message) => `${roleLabel(message.role)}：${message.content}`)
    .join("\n\n");
  return `${hintBlock}${contextHeader}\n\n${context}\n\n${finalHeader}\n${prompt}`;
}
...
function compactAgentReply(output: string, fallback: string) {
  ...
  const lines = clean
    ...
    .filter((line) => !/^DeepSeek stream started:/i.test(line))
    ...
    .filter((line) => !/^正在请求[。.]*$/i.test(line))
    .filter((line) => !/^requesting[。.]*$/i.test(line));
...
function formatConciseRunReply(capture: RunCapture, exit: { exitCode?: number; signal?: number; finalOutput?: string }, language: AppLanguage) {
  ...
  const output = exit.finalOutput?.trim() ? exit.finalOutput : capture.output;
  if (!ok) return compactAgentReply(output, copy.failedShort);
  return compactAgentReply(output, copy.completedShort);
}
...
function App() {
  ...
  const [settingsSaveMessage, setSettingsSaveMessage] = useState<{ok: boolean, text: string} | null>(null);
  ...
  const saveSettings = useCallback(async () => {
    if (apiKey.trim()) {
      ...
      if (!keyResult.ok) {
        ...
        setSettingsSaveMessage({ ok: false, text: keyResult.error || t.settings.apiKeySaveFailed });
        return;
      }
    }
    ...
    setSettingsSaveMessage({ ok: true, text: t.remote.saved });
  ...
  const chooseCustomBinary = useCallback(async () => {
    const selected = await desktop.chooseFile([
      { name: "DeepSeek executable", extensions: ["exe"] }
    ]);
    if (selected) {
  ...
  const launch = useCallback(async (action?: LaunchAction, promptOverride?: string, captureSessionId?: string, replyMessageId?: string) => {
    ...
    let result: Awaited<ReturnType<typeof desktop.startTerminal>>;
    try {
      ...
      if (replyMessageId) {
        const message: ChatMessage = { id: replyMessageId, role: "assistant", content: messageText };
        if (terminalSessionId) {
          commitConversationStore((current) => updateConversationSession(current, terminalSessionId, language, (session) => ({
            ...session,
            updatedAt: new Date().toISOString(),
    ...
    if (!result.ok) {
      ...
      terminalRunSessionIdRef.current = "";
      if (replyMessageId) {
        const messageText = result.error || t.status.launchFailed;
        const message: ChatMessage = { id: replyMessageId, role: "assistant", content: messageText };
        if (terminalSessionId) {
          commitConversationStore((current) => updateConversationSession(current, terminalSessionId, language, (session) => ({
            ...session,
    ...
    if (result.ok && typeof result.finalOutput === "string") {
      const capture: RunCapture = runCaptureRef.current || {
        action: launchAction,
        prompt,
        sessionId: terminalSessionId,
        replyMessageId,
      ...
      if (terminalSessionId) {
        commitConversationStore((current) => updateConversationSession(current, terminalSessionId, language, (session) => ({
          ...
          messages: replyMessageId
            ? session.messages.map((candidate) => candidate.id === replyMessageId ? message : candidate)
            : [...session.messages, message]
        })));
      }
      setMessages((current) => replyMessageId
        ? current.map((candidate) => candidate.id === replyMessageId ? message : candidate)
        : [...current, message]);
      return;
    }
    if (result.runtime) {
    ...
    if (result.ok && shouldCapture && result.sessionId) {
      const backendSessionId = result.sessionId;
      const pollStartedAt = Date.now();
      const pollTerminalResult = async () => {
        if (!runCaptureRef.current || runCaptureRef.current.replyMessageId !== replyMessageId) return;
        try {
          const completed = await desktop.getTerminalResult(backendSessionId);
          ...
          // Event delivery remains the primary path; polling is just a fallback.
        }
        if (Date.now() - pollStartedAt < 180000) {
          window.setTimeout(pollTerminalResult, 1000);
        }
      };
      window.setTimeout(pollTerminalResult, 1000);
    }
  ...
  const toggleMcp = useCallback((id: string) => {
    setSettings((current) => {
      ...
      const enabledMcpServers = Array.from(enabled);
      return {
        ...current,
        enabledMcpServers,
        mcpEnabled: enabledMcpServers.length > 0 || current.mcpEnabled
      };
    });
  }, []);
  ...
  const sendPrompt = useCallback(async () => {
    ...
    setAgentPrompt("");
    await launch(launchAction, buildConversationPrompt(messages, prompt, language, settings), conversationStore.activeSessionId, replyMessageId);
  }, [agentMode, agentPrompt, commitConversationStore, conversationStore.activeSessionId, language, launch, messages, settings, t]);
  ...
  const conversationTurnCount = messages.filter((message) => message.role === "user").length;
  const conversationTurnLabel = language === "zh"
    ? `当前会话 · ${conversationTurnCount} 轮`
    : `Current session · ${conversationTurnCount} turns`;
      ...
      <section className="conversation-main">
        <header className="conversation-topbar">
          <div className="topbar-title">
            ...
            <span className="conversation-turn-count">{conversationTurnLabel}</span>
          </div>
        ...
        <aside className="inspector-panel">
          ...
          {inspectorPanel === "settings" ? (
            <div className="inspector-content">
              ...
              {settingsSaveMessage ? (
                <small className={`settings-save-msg ${settingsSaveMessage.ok ? "ok" : "error"}`}>
