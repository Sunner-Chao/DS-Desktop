...
declare global {
  interface Window {
    ...
    deepseekDesktop: {
      ...
      startTerminal: (options: LaunchOptions) => Promise<{ ok: boolean; error?: string; runtime?: RuntimeCheck; pid?: number; sessionId?: string; finalOutput?: string; exitCode?: number }>;
      getTerminalResult: (sessionId: string) => Promise<{ sessionId: string; exitCode: number; finalOutput: string } | null>;
      stopTerminal: () => Promise<{ ok: boolean }>;
      sendTerminalInput: (data: string) => void | Promise<void>;
      resizeTerminal: (size: { cols: number; rows: number }) => void | Promise<void>;
      ...
      pushUpdateNotice: (payload: UpdatePushPayload) => Promise<{ ok: boolean; error?: string; notice?: UpdateNotice }>;
	      ...
	      onTerminalExit: (callback: (exit: { exitCode: number; signal?: number; finalOutput?: string }) => void) => () => void;
